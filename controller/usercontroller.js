...
const productdet = async (req, res) => {
    try {
        ...
        const productdata = await product.findOne({ _id: productid }).populate('category'); // Assuming 'category' is a reference in the product schema

        // Find the user's cart and wishlist
        ...
        res.render('product-details', { products: productdata, category: categorydata, cartCount, wishcount });
    } catch (error) {
...
const referal = async (req, res) => {
    try {
        ...
        // Find or create wallet for the current user
