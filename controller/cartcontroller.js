    ...
    try {
        ...
        if (req.session.coupon) {
            ...
            if (couponData) {
                ...
                usedCouponCode = couponData.couponCode; // Store the used coupon code
            }
        }

        const coupons = await Coupon.find().sort({ created: -1 });
        const availableCoupons = coupons.filter(coupon => coupon.couponCode !== usedCouponCode); // Filter out the used coupon
        ...
        res.render('checkout', {
            ...
            discountAmount,
            discountTotal,
            coupons: availableCoupons // Pass the filtered coupons to the template
        });
