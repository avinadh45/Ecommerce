...
const productlist = async(req,res)=>{
    try {
        ...
        // console.log(count,"cout for pagination");
...
const updateProduct = async (req, res) => {
    ...
    try {
        ...
        if (!req.files || req.files.length === 0) {
            ...
            if (!categorys) {
                console.log("kizhakepaadam enna athimanoharamaaya gramam");
                return res.render('editproduct',{ error: 'Category not found' });
            }
   console.log(categorys,"ballelakha ballelakha");
            ...
            if (!updatedProduct) {
                console.log("baba baba samaadhanam");
                return res.render('editproduct',{  error: 'Product not found' });
            }
...
console.log(categorys,"jum barabar barabar");
        const categoryId = categorys._id;
        ...
        for (let i = 0; i < req.files.length; i++) {
            const filename = req.files[i].filename;
            if (!req.files[i].mimetype.startsWith('image')) {
                console.log("jai baba roooh baba");
        ...
        if (!updatedProduct) {
            console.log("update baba");
            return res.render('editproduct',{error: 'Product not found' });
        }
// else{
//         res.render('editproduct',{ product:updatedProduct,category:categorys,categorys,success: true, data: updatedProduct });
// }
res.redirect('/admin/productlist')
    } catch (error) {
        console.error('Error updating product:', error);
       // res.status(500).render('editproduct',{error: 'Server Error' });
    }
};
