...
const verifylogin = async (req, res) => {
  ...
  try {
    ...
    if (userData) {
      ...
      if (isPasswordValid) {
        // Password matches, store user information in session
  ...
  try {
    ...
    const detailedOrders = await Promise.all(orderdata.map(async order => {
      ...
      products.forEach(product => {
        console.log(`Product ID: ${product._id}, Offer: ${product.offer}, Price: ${product.price}`);
        if (product.offer > 0) {
    ...
    doc.fontSize(10).text(`Overall Order Amount: ₹${overallOrderAmount}`).moveDown();
    doc.fontSize(10).text(`Total Discount: ₹${totalDiscount}`).moveDown();
    doc.fontSize(10).text(`Total Coupon Deduction: ₹${totalCouponDeduction}`).moveDown().moveDown();

    const table = {
      headers: ['Order ID', 'Billing Name', 'Date', 'Total', 'Discount', 'Coupon Deduction', 'Payment Status', 'Payment Method', 'Order Status'],
...
const downloadexcel = async (req, res) => {
  try {
    ...
    worksheet.addRow(['Order ID', 'Billing Name', 'Date', 'Total', 'Discount', 'Coupon Deduction', 'Payment Status', 'Payment Method', 'Order Status']);
...
// const combinedChart = async (req, res) => {
//   try {
//     const timeRange = req.query.timeRange || 'monthly';
//     console.log(timeRange, "hi from the timerange");
...
//     res.status(500).json({ error: 'Failed to fetch combined chart data' });
//   }
// };
