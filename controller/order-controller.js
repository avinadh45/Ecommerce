    ...
    try {
        ...
        if (usercart) {
            ...
            if (coupon) {
                ...
                const couponUsage = userdata.Coupon.find(a => a.couponCode === coupon);
                if (couponUsage) {
                    return res.status(400).json({ success: false, message: 'You have already used this coupon.' });
                }

                userdata.Coupon.push({ couponCode: coupon, usagecount: 1 });
                await userdata.save();
...
const removecoupon = async (req, res) => {
    try {
        ...
        console.log(req.session.coupon, "session for the cart remove");

        
        let userCart = req.session.user.cart;
        console.log(userCart, "for the user");

        
        if (!userCart) {
            const userId = req.session.user._id;
            console.log(userId,"user cart for he coupon");
            userCart = await Cart.findOne({ user_id: userId });
            console.log(userCart, "Fetched from database");
            if (!userCart) {
        ...
        res.status(200).json({ success: true, subtotal: subtotal });
    } catch (error) {
